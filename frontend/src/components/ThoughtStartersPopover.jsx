...
export function ThoughtStartersPopover({ prompts, selectedPrompt, onSelectPrompt, useInlineHover = false }) {
  return (
    <Card
      className="border border-stroke pt-4 pr-4 pb-4 pl-3 overflow-y-auto"
      style={{
        ...
        {prompts.map((prompt) => {
          ...
          return (
            <button
              ...
              className={`flex items-start gap-1.5 w-full text-left text-sm font-serif text-ink leading-relaxed py-2 pl-0 pr-2 rounded transition-colors duration-150 ${!useInlineHover && (isSelected ? 'bg-muted/30' : '')} ${!useInlineHover ? 'hover:bg-muted/60' : ''}`}
            ...
            >
              <span className="flex-shrink-0 w-3.5 flex items-center justify-center" style={{ height: '1.625em', minHeight: '1.625em' }}>
